...
const makeCourse = (): Course =>
  new Course({ id: 'c1', title: 'T', description: 'D', coverImageUrl: null, state: 'draft', createdBy: 'u1', semesterCount: 0, publishedAt: null, deletedAt: null, createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z' });
...
describe('CreateSemesterUseCase', () => {
  ...
  it('creates a semester and increments semesterCount', async () => {
    ...
    const semester = await useCase.execute({ courseId: 'c1', title: 'Sem 1', description: '' });
  ...
  it('throws 404 when course not found', async () => {
    ...
    await expect(useCase.execute({ courseId: 'c1', title: 'S', description: '' })).rejects.toMatchObject({ status: 404, errorCode: 'COURSE_NOT_FOUND' });
  });
});
